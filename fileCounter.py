...
def calcRampUp(docLines, codeLines, testLines):
    ...
    docLines = docLines if docLines > 0 else 1
    codeLines = codeLines if codeLines > 0 else 1
    testLines = testLines if testLines > 0 else 1
    ratio = docLines/adjLines
    logValue = math.log(adjLines, 100) 
    logValue = logValue if logValue > 0 else 1
    rampUp = math.tanh(3 * ratio / logValue)
    return(rampUp)
